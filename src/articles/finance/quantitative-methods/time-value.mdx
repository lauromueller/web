...
For us to figure out the actual rate we have to use and how many times we should compound the interest, we should adjust the stated annual interest rate. The periodic rate is given by the stated annual interest rate $$ r_s $$ divided by the frequency of compounding:

<Definition title="Test definition">

$$
FV_{N=n} = Y \left( 1 + \dfrac{r_s}{m} \right) ^{mn}
$$

</Definition>

If we consider an example where an initial amount of $$ Y = \$ 1.000,00 $$ is invested in a paper that offers $$ 10 \% $$ as the stated annual interest rate compounded monthly and a time to maturity of $$ 1 $$ year, we obtain:

<Example title="Test example">

$$
FV_{N=1} = \$ 1.000{,}00 \left( 1 + \dfrac{0.1}{12} \right) ^{12} \newline
FV_{N=1} = \$ 1.000{,}00 \left( 1.008333 \right) ^{12} \newline
FV_{N=1} = \$ 1.104{,}71
$$

</Example>

As it can be seen, the effective annual interest rate is slightly higher than the stated annual rate. This happens due to the compounding effect of interest rates, and it is important to highlight that there is nothing wrong with our calculations. We are **not** converting from annual to monthly rates when we divide the stated annual interest rate by $$ 12 $$. The stated annual rate is a symbolic value that is stated in paper, but in reality it should be converted into the effective interest rate by adjusting it to the frequency of compounding.

### Continuous compounding

Continuous compounding means that we compound the interest rate continuously over time. In terms of the formula above [GET THE DEFINITION!!!!], this means $$ m \rightarrow \infty $$. We are then interested in finding the actual amount of interest that will be paid over the period $$ N $$ when $$ m $$ is infinitely small. In other words, we are interested in:

$$
FV_{N=n} = \lim_{m \rightarrow \infty} Y \left( 1 + \dfrac{r_s}{m} \right) ^{mn}
$$
...
(If you are not familiar with the proof, there are several ways of demonstrating that $$ \lim_{m \rightarrow \infty} \left( 1 + \dfrac{x}{m} \right) ^{m} = e^x $$, just look for the expression on Google and you should come across the most common derivations of the result).

Coming back to our previous example, if we were to consider the same investment ($$ N = 1 $$, $$ Y = \$ 1.000{,}00 $$, and $$ r_s = 0.1 $$), continuous compounding would yield us the following sum at the end of the period:

<Example title="Test example">

$$
FV_{N=1} = \$ 1.000{,}00 e^{0.1} \newline
FV_{N=1} = \$ 1.105{,}17
$$

</Example>

The final sum of continuous compounding is slightly higher than the sum of discrete compounding. In fact, it can be shown that, everything else being equal, continuous compounding offers an upper limit to the return of an investment for a given time period and stated interest rate.

## The future value of multiple cash flows

We are now able to determine the future value of a single cash flow, and this can be easily extended for a series of cash flows. Suppose that we decide to invest $$ \$ 1.000,00 $$ every month in an application that pays an interest rate of $$ 1\% $$ per month at the end of each month. How much would we have at the end of $$ 12 $$ months?

<Example title="Example XX: Compounding over 12 months" alignEquationsLeft={true}>

To state the problem more formally, let's define each of the variables of our formula. Remember, we are interested in calculating how much money we will have at the end of a $$ 12 $$-month investment with monthly contributions of $$ \$ 1.000,00 $$.
...
We know that our initial application $$ Y $$ at time $$ t = 1 $$ will collect interest right from the beginning. In other words, it will collect interest over $$ 11 $$ months of our application. For this example, I will assume our periods vary from $$ 1 $$ to $$ 12 $$. As a consequence, we have $$ 11 $$ "in-between" periods. For us to better understand this idea, let's quickly consider a 3-period example. Have a look at the picture below:

<img src="./assets/time-value-3.png" />

As you can see, the fact that we have three periods leads to the first deposit compounding over two period deltas, the second compounding over one period delta, and the third compounding over zero period deltas. If we take this reasoning and apply to our current calculations, we will conclude that the amount deposited at $$ t_1 $$ will compound over 11 periods, the amount from $$ t_2 $$ over 10 periods, and so on. The amount at $$ t_12 $$ will compound over zero periods (in other words, we simply add the unchanged amount to our final sum).

$$
FV_{N=11}^{Y_1} = \$ 1.000{,}00 (1.1)^{11}\newline
FV_{N=10}^{Y_2} = \$ 1.000{,}00 (1.1)^{10}\newline
FV_{N=9}^{Y_3} = \$ 1.000{,}00 (1.1)^{9}\newline
...\newline
FV_{N=1}^{Y_{11}} = \$ 1.000{,}00 (1.1)^{1}\newline
FV_{N=0}^{Y_{12}} = \$ 1.000{,}00 \newline
$$

Our final value is then given by the sum of all these values:

$$
FV = \sum_{i=1}^{12} FV_{N=12-i}^{Y_i}\newline
FV = \sum_{i=1}^{12} \$ 1.000{,}00 (1.1)^{i-1}\newline
FV = \$ 21.384{,}28
$$

In the end of 12 months, we will have accumulated a total of $$ \$ 21.384{,}28 $$, which is substantially higher than if we had just collected $$ \$ 1.000{,}00 $$ per month without any interest.

</Example>

### Ordinary annuities

The short example above shows how compound interest can lead to substantial increases on our wealth if we invest our money early. The example also served to illustrate the basic concept behind **ordinary annuities**.

<Definition title="Definition XX: Ordinary annuity">

An ordinary annuity is a series of equal cash flows $$ A $$, which are incurred over equally spaced time intervals amounting to a total of $$ N $$ periods. The future value of an annuity is given by:

$$
FV_{N=N} = A \left( \dfrac{(1+r)^{N} - 1}{r} \right)
$$

Which corresponds to the sum of the first N terms of a geometric sequence with common ratio $$ r $$.

</Definition>

### Working with unequal cash flows

While equal cash flows may happen under certain circumstances (for example, if you determine that you will save a fixed amount from your monthly income and deposit it in an investment), it is much more common to come across unequal cash flows.

The mechanism for working with unequal cash flows is very similar to that of working with equal cash flows: finding the future value of a series of cash flows requires not more than finding the exact future value of each cash flow and adding them. Unfortunately, due to the fact that we have different periodic cash flows, we cannot reduce the process to a closed-form equation such as the one from ordinary annuities.
