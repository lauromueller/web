...
![Compounding payments over three periods](./assets/time-value-3.png)

Continuing with our example, for the amount that is deposited in the second period ($$ t = 2 $$), interest will be collected over $$ 11 $$ months rather than the full period. In other words, each monthly deposit further into the future will collect interest over a shorter period. At the end of $$ 12 $$ months we will have the following:

$$
FV_{N=12}^{Y_0} = \$ 1.000,00 (1.1)^{12}\newline
FV_{N=11}^{Y_1} = \$ 1.000,00 (1.1)^{11}\newline
FV_{N=10}^{Y_2} = \$ 1.000,00 (1.1)^{10}\newline
...\newline
FV_{N=2}^{Y_{10}} = \$ 1.000,00 (1.1)^{2}\newline
FV_{N=1}^{Y_{11}} = \$ 1.000,00 (1.1) \newline
$$

Our final value is then given by the sum of all these values:

$$
FV = \sum_{i=0}^{11} FV_{N=12-i}^{Y_i}\newline
FV = \sum_{i=0}^{11} \$ 1.000,00 (1.1)^{i+1}\newline
FV = \sum_{i=0}^{11} \$ 1.000,00 (1.1)^{i+1}\newline
$$
